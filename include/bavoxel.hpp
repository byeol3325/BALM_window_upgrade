...
public:
  ...
  void parallel_evaluate_only_residual(const vector<IMUST>& xs, double& residual)
  {
      residual = 0;
      int gps_size = plvec_voxels.size();
      ...
      std::vector<double> partial_residuals(THREAD_NUM, 0);
      int items_per_thread = gps_size / THREAD_NUM;

      for (int i = 0; i < THREAD_NUM; ++i) {
          int start = i * items_per_thread;
          int end = (i == THREAD_NUM - 1) ? gps_size : (i + 1) * items_per_thread;
          threads[i] = std::thread(worker, start, end, std::ref(partial_residuals[i]));
      }
...
public:
  ...
  double only_residual(vector<IMUST> &x_stats, VOX_HESS &voxhess, vector<IMUST> &x_ab, string opt)
  {
    double residual1 = 0, residual2 = 0;
    if (opt == "parallel") {
        voxhess.parallel_evaluate_only_residual(x_stats, residual2); // **** success
    }
    else {
  ...
  void damping_iter(vector<IMUST> &x_stats, VOX_HESS &voxhess, string opt)
  {
    ...
    for(int i=0; i<10; i++)
    {
      ...
      residual2 = only_residual(x_stats_temp, voxhess, x_ab, opt);
